...
function CemberinCevresi(yarıçap) {
  return (CemberinCevresi = 2 * pi * yarıçap);
}
...
function CemberinAlani(yarıçap, pi) {
  return (CemberinAlani = pi * Math.pow(yarıçap, 2));
}
...
ucetambolunenler.forEach((item) => {
  ucebolunenlerintoplami += item;
});
...
for (let i = 0; i < yeniSayilar.length; i++) {
  if (obje[yeniSayilar[i]] == undefined) {
